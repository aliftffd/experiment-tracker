...
impl Database {
    /// Insert a new run, return the created run
    ...
    pub fn get_all_runs(&self) -> Result<Vec<Run>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, name, status, log_path, created_at, updated_at, notes FROM runs ORDER BY updated_at DESC",
            )?;
    ...
    /// Update run status
    pub fn update_run_status(&self, id: i64, status: &RunStatus) -> Result<()> {
        self.conn.execute(
    ...
    pub fn run_exists_by_path(&self, log_path: &str) -> Result<bool> {
        let count: i64 = self.conn.query_row(
            "SELECT COUNT(*) FROM runs WHERE log_path = ?1",
